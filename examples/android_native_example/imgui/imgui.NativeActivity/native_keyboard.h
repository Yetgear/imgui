...
extern void showKeyboard(ANativeActivity *m_activity, bool bShow) {
	// Attaches the current thread to the JVM.
	jint lResult;
	jint lFlags = 0;

	JavaVM* lJavaVM = m_activity->vm;
	JNIEnv* lJNIEnv = m_activity->env;
	...
	jobject lNativeActivity = m_activity->clazz;
